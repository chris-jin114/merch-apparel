...
export class KidsPage implements OnInit, OnDestroy {
  ...
  ionViewWillEnter() {
    this.titleService.setTitle('Arena Apparel | Kids');
    this.getData();
  }
  
  getData() {
    this.subs.sink = this.categoryState.getCategories().pipe(
      map((categories: Category[]) => categories.find(item => item.category_name.toLowerCase() === 'kids'))
