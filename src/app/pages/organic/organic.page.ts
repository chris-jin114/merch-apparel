...
export class OrganicPage implements OnInit, OnDestroy {
  ...
  ionViewWillEnter() {
    this.titleService.setTitle('Arena Apparel | Organic');
    this.getData();
  }
  
  getData() {
    this.subs.sink = this.categoryState.getCategories().pipe(
      map((categories: Category[]) => categories.find(item => item.category_name.toLowerCase() === 'organic'))
