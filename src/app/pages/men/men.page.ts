...
export class MenPage implements OnInit, OnDestroy {
  ...
  ionViewWillEnter() {
    this.titleService.setTitle('Arena Apparel | Men');
    this.getData();
  }

  getData() {
    this.subs.sink = this.categoryState.getCategories().pipe(
      map((categories: Category[]) => categories.find(item => item.category_name.toLowerCase() === 'men'))
